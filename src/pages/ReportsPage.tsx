...
import { mockWeeklyReports, cityConfig } from "@/lib/mock-data";
...
export default function ReportsPage() {
  ...
  const filtered = mockWeeklyReports.filter((r) => {
    ...
    <SiteLayout>
      ...
      <section className="py-8">
        <div className="container max-w-3xl space-y-4">
          {filtered.map((report) => {
            const topHeadlines = report.sections.top_headlines as string[] | undefined;
            return (
...
export function ReportDetailPage() {
  const { id } = useParams();
  const report = mockWeeklyReports.find((r) => r.id === id);
    ...
    return (
      <SiteLayout>
        <div className="container py-20 text-center">
          ...
          <Link to="/reports" className="mt-4 inline-flex items-center gap-1 text-primary hover:underline">
  ...
  return (
    <SiteLayout>
      <section className="bg-primary py-10">
        <div className="container">
          <Link to="/reports" className="mb-4 inline-flex items-center gap-1 text-sm text-primary-foreground/70 hover:text-primary-foreground">
          ...
          <p className="mt-2 text-sm text-primary-foreground/50">Published {formatDate(report.published_at)}</p>
        </div>
      </section>

      <section className="border-b bg-background py-3">
        <div className="container flex gap-3">
          <button onClick={() => window.print()} className="flex items-center gap-1 rounded-lg border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-secondary">
            <Printer className="h-3.5 w-3.5" /> Print
          </button>
          <button onClick={() => navigator.clipboard.writeText(window.location.href)} className="flex items-center gap-1 rounded-lg border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-secondary">
      ...
      <section className="py-8">
        <div className="container max-w-3xl space-y-8">
          {sections.map((section) => {
            const content = (report.sections as Record<string, unknown>)[section.key];
      ...
      <section className="bg-accent py-8">
        <div className="container text-center">
          ...
          <form className="mx-auto mt-4 flex max-w-md gap-3" onSubmit={(e) => { e.preventDefault(); setEmail(""); }}>
