...
import { ArrowRight, Calendar, AlertTriangle, Users, FileText } from "lucide-react";
import { SiteLayout } from "@/components/SiteLayout";
import { PriorityBadge } from "@/components/PriorityBadge";
import { CategoryBadge } from "@/components/CategoryBadge";
import { useCityConfig, useWeeklyReports, useIssues, useMeetings, useHighPriorityFlags, useCommunityPosts } from "@/hooks/use-data";
...
export default function HomePage() {
  ...
  const stats = [
    ...
    { label: "Open Issues", value: openIssuesCount, icon: FileText },
    { label: "Active Alerts", value: activeAlerts.length, icon: AlertTriangle },
    { label: "Last Updated", value: "Today", icon: Users },
  ];
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {recentIssues.map((issue) => (
              <div key={issue.id} className="group rounded-lg border bg-card p-5 transition-shadow duration-150 hover:shadow-md">
                ...
                <p className="text-xs text-muted-foreground">{issue.created_at ? formatDate(issue.created_at) : ""}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="space-y-3">
                ...
                <div key={meeting.id} className="flex items-center gap-4 rounded-lg border bg-card p-4">
                  <div className="text-center">
                    <p className="text-xs font-medium uppercase text-muted-foreground">
                      {meeting.meeting_date ? new Date(meeting.meeting_date).toLocaleDateString("en-US", { month: "short" }) : ""}
                    </p>
                    <p className="text-2xl font-bold text-foreground">
                      {meeting.meeting_date ? new Date(meeting.meeting_date).getDate() : ""}
                    </p>
                  </div>
              ...
              {upcomingMeetings.length === 0 && (
                <div className="rounded-lg border bg-card p-6 text-center text-muted-foreground">No upcoming meetings this week.</div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-3">
                {activeAlerts.map((alert) => (
                  <div key={alert.id} className="rounded-lg border-l-4 border-l-destructive bg-destructive/5 p-4">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{alert.flagged_at ? formatDate(alert.flagged_at) : ""}</p>
                  </div>
                ))}
              </div>
            ) : (
      ...
      <section className="bg-accent py-12">
        <div className="container text-center">
          <h2 className="text-2xl font-bold text-accent-foreground">
            Get civic alerts for {config?.city_name ?? "your city"}
          </h2>
          <p className="mt-2 text-sm text-accent-foreground/70">Weekly digest + breaking alerts. No spam.</p>
          <form className="mx-auto mt-6 flex max-w-md gap-3" onSubmit={handleSubscribe}>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required className="flex-1 rounded-lg border border-accent-foreground/20 bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" />
            <button type="submit" className="rounded-lg bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground transition-colors duration-150 hover:bg-primary/90">Subscribe Free</button>
          </form>
        </div>
      </section>
    </SiteLayout>
