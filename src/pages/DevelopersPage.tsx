...
import { cityConfig, mockDevelopers } from "@/lib/mock-data";
...
export default function DevelopersPage() {
  ...
  const [selectedDev, setSelectedDev] = useState<typeof mockDevelopers[0] | null>(null);
  ...
  const filtered = mockDevelopers.filter((d) => {
  ...
  const repeatCount = mockDevelopers.filter((d) => d.appearance_count >= 3).length;
  const thisMonth = mockDevelopers.filter((d) => new Date(d.created_at).getMonth() === new Date().getMonth()).length;
    ...
    <SiteLayout>
      <section className="bg-primary py-10">
        <div className="container">
          ...
          <p className="mt-2 text-primary-foreground/70">Every development application filed in {cityConfig.city_name}, with repeat applicant flagging.</p>
        </div>
      </section>
      ...
      <section className="border-b bg-background py-4">
        <div className="container flex flex-wrap gap-6 text-sm">
          <span className="font-medium text-foreground">Total Applications: {mockDevelopers.length}</span>
          <span className="font-medium text-accent-foreground">⚠️ Repeat Applicants: {repeatCount}</span>
          <span className="font-medium text-foreground">Active: {mockDevelopers.filter((d) => d.status === "active").length}</span>
          <span className="font-medium text-foreground">This Month: {thisMonth}</span>
        </div>
      </section>
                  ...
                  <tr key={dev.id} onClick={() => setSelectedDev(dev)} className="cursor-pointer border-t transition-colors duration-150 hover:bg-section">
                    ...
                    <td className="px-4 py-3">
                      <span className="flex items-center gap-1">
                        {dev.appearance_count >= 3 && <AlertTriangle className="h-3.5 w-3.5 text-accent" />}
                        <span className={dev.appearance_count >= 3 ? "font-semibold text-accent-foreground" : "text-muted-foreground"}>
                          {dev.appearance_count}
                        </span>
                      </span>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{formatDate(dev.created_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md border-l bg-background shadow-xl">
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b bg-primary p-5">
              ...
              <button onClick={() => setSelectedDev(null)} className="rounded p-1 text-primary-foreground hover:bg-primary-foreground/10">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-5">
              <dl className="space-y-3 text-sm">
                ...
                <div><dt className="text-muted-foreground">Status</dt><dd><StatusBadge status={selectedDev.status} /></dd></div>
                <div><dt className="text-muted-foreground">Appearances</dt><dd className="flex items-center gap-1 text-foreground">{selectedDev.appearance_count >= 3 && <AlertTriangle className="h-3.5 w-3.5 text-accent" />} {selectedDev.appearance_count}</dd></div>
                <div><dt className="text-muted-foreground">Filed</dt><dd className="text-foreground">{formatDate(selectedDev.created_at)}</dd></div>
